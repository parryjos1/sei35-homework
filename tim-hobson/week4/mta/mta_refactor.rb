def plan_trip
  stations = { n: [ 'Times Square', '34th', '28th', '23rd', 'Union Square', '8th' ], l: [ '8th', '6th', 'Union Square', '3rd', '1st' ], six: [ 'Grand Central', '33rd', '28th', '23rd', 'Union Square', 'Astor Place' ] }
  ...
  def plan_trip from_line, from_station, to_line, to_station # takes in all the tip data
    if from_line == to_line
      ...
      puts "Board at #{from_station} station on the #{from_line.upcase} through the following #{single_line_trip.length - 1} stop(s)", single_line_trip[1..-1].join(' - ')
      ...
      puts "Board at #{from_station} station on the #{from_line.upcase} line and travel through the following #{to_interchange.length - 1} stop(s)", to_interchange[1..-1].join(' - ')
      puts "Change at Union Square to the #{to_line.upcase} line"
      from_interchange = line_trip to_line, "Union Square", to_station
      totalstops = (to_interchange.length - 1) + (from_interchange.length - 1)
      puts "Travel through the following #{from_interchange.length - 1} stop(s) to your destination, for a total of #{totalstops} stops", from_interchange[1..-1].join(' - ')
    end
  end
end
...
puts "Boarding on the #{from_line.upcase} line at #{from_station}, headed to #{to_station} on the #{to_line.upcase} line."
