...
const tictactoe = {
    ...
    initialize: function() {
        ...
        computerWins.innerText = `Mr. Robot Jnr: ${this.computerWins}`;
...
function minimax(board, player, depth=0) {
    ...
    for(let x = 0; x < availableSpots.length; x++) {
        ...
        //setting empty spot on the board to current player
            ...
            let result = minimax(child, "Mr. Robot Jnr", depth+1);
            move.score = result.score;
        }
...
async function smartComputer() {
    ...
    if(tictactoe.endgame === true) {
        ...
        let bestMove = minimax(tictactoe.entryArray, "Mr. Robot Jnr");
...
async function computerTakeTurn() {
    ...
    if(tictactoe.endgame === true) {
        ...
        if(tictactoe.computerSymbol === 'O') {
            ...
            box[randomIndex].classList.toggle("cross-bg");
        }
...
function takeTurn(event) {
    if(tictactoe.currentTurn === 'Mr. Robot Jnr') {
        document.querySelector('.overlay').style.display = 'block';
        document.querySelector('#endgame').style.display = 'block';
