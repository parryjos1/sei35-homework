console.log("YERRRR reverse function test!!!");
...
const mtaMap = {
  ...
  // let newStartIndex;
  // let newEndIndex;
  let firstLegReversed;
  let secondLegReversed;

  reverseIt = function ( departureLineArray, startStation , destinationLineArray, finishStation ) {


    firstLegReversedArray = departureLineArray.reverse();
    secondLegReversedArray = destinationLineArray.reverse();  //reverses array

    // console.log(reversedArray);

    newStartIndex = firstLegReversedArray.indexOf( startStation );  //finds new index positions
    newEndIndex = secondLegReversedArray.indexOf( finishStation ) //of stops

    // console.log(newStartIndex);
    // console.log(newEndIndex);

    firstLegReversed = firstLegReversedArray.slice(newStartIndex +1, newEndIndex +1);
    secondLegReversed = secondLegReversedArray.slice(newStartIndex +1, newEndIndex +1);
    // console.log(journey);
  ...
  journeyPlanner = function (startLine, startStation, finishLine, finishStation) {
    ...
    if (departureStationIndex > destinationStationIndex && startLine === finishLine) {
      ...
      // return ( `To get there you must travel ${journey.length} stops, these include ${journey}, exiting at ${destinationStation} `);
          ...
          //
              ...
              return ( `To get there you must travel ${simpleJourney.length} stops, these include ${simpleJourney}, exiting at ${destinationStations} `);
