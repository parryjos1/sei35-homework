console.log("YERRRR reorganisation attempt!!!");
...
const mtaMap = {
  ...
  let destinationStationIndex;
      ...
      for (let key in mtaMap.lines) {
        ...
        const stopIndex = endStop.indexOf( finishStop );
            ...
            if (endLine === finishLine && endStop.includes(finishStop)) { // checks line and stop is on line
               ...
               destinationLine = endLine;
               destinationLineArray = endStop;
               destinationStation = finishStop;
    ...
    reverseIt = function () {
      ...
      const reversedArray = departureLineArray.reverse()  //reverses array

      // console.log(reversedArray);

      const newStartIndex = reversedArray.indexOf( departureStation );  //finds new index positions
      const newEndIndex = reversedArray.indexOf( destinationStation ) //of stops
    ...
    changeLine = function () {
        ...
        const changeIndexStart = departureLineArray.indexOf('Union Square')  //finds index of union square
        const changeIndexEnd = destinationLineArray.indexOf('Union Square')
        //
        // console.log(changeIndexStart);
        // console.log(changeIndexEnd);


        const firstLeg = departureLineArray.slice( departureStationIndex +1, changeIndexStart +1); // finds remaining
        const secondLeg = destinationLineArray.slice(changeIndexEnd +1, destinationStationIndex +1);   //stops on journey
        //
        // console.log(firstLeg);
        // console.log(secondLeg);
              //
    ...
    journeyPlanner = function (startLine, startStop, finishLine, finishStop) {

        findStartPosition(startLine, startStop);

        console.log(`You are currently at ${startStop}, on the ${departureLine}`);
        console.log(departureStationIndex);

        findEndPosition(finishLine, finishStop);

        console.log(`You are travelling to ${finishStop}, on the ${destinationLine}`);
        console.log(destinationStationIndex);
        ...
        } else if ( departureStationIndex > destinationStationIndex  ){
